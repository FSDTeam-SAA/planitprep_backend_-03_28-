            ...
            <div class="col-sm-12">
                <div class="card ">
                    <div class="card-header">
                        ...
                        <a href="{{ route('admin.supplement-types.create') }}" class="btn btn-primary d-inline-flex align-item-center float-end">
                            <i class="ti ti-plus f-18"></i> Add Supplement Type
                        </a>
                    </div>
                    <div class="card-body">
...
<script type="text/javascript">
    var table = $('#jsource-table').DataTable({
        ajax: "{{ route('admin.supplement-types') }}",
        processing: true,
        serverSide: true,
        deferRender: true,
        order: [
        ...
        swalWithBootstrapButtons
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('admin.supplement-types.destroy') }}",
                        type: 'POST',
                        data: {
                            "ids": ids
                        },
    ...
    $(document).on('click', '.active', function() {
        ...
        swalWithBootstrapButtons
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('admin.supplement-types.updateActive') }}",
                        type: 'POST',
                        data: {
                            id: id,
                            value: val
