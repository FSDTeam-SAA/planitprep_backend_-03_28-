...
class DietTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    <a href="'.route('admin.diet-types.edit', $row->id).'" class="edit avtar avtar-xs btn-link-success btn-pc-default"><i class="ti ti-edit-circle f-18"></i></a>
                    </li>
                    <li class="list-inline-item align-bottom" data-bs-toggle="tooltip" title="Edit">
        ...
        return view('admin.diet-types.index', compact('title'));
    }

    public function create()
    {
        $action = 'Create';
        $title = 'Create Diet Type';

        return view('admin.diet-types.diet-type', compact('action', 'title'));
    }

    private function _form_validation($request)
    {
        $rules = [
            'name' => 'required|string|max:50|unique:diet_types,name,'.($request->id ?? 'null'),
        ];
        $messages = [
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.diet-types')->with('success', 'Diet Type saved successfully.');
    }
    ...
    public function edit($id)
    {
        ...
        if ($image != '') {
            if (! Storage::disk('public')->exists('diet-type/'.$image)) {
                $image = '';
            }
        } else {
            $image = '';
        ...
        return view('admin.diet-types.diet-type', compact('action', 'title', 'record'));
    }
    ...
    public function update($id, Request $request)
    {
        ...
        DietType::where('id', $id)->update($data);

        return redirect()->route('admin.diet-types')->with('success', 'Changes saved successfully.');
    }
    ...
    public function destroy(Request $request)
    {
        ...
        DietType::whereIn('id', $ids)->delete();
        if (count($ids) > 1) {
            $msg = 'Diet types deleted successfully';
        } else {
            $msg = 'Diet Type deleted successfully';
        }
    ...
    public function updateActive(Request $request)
    {
        ...
        DietType::where('id', $id)->update(['active' => $active]);
